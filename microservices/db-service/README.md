...
```sh
sudo docker run -d -e TZ=Asia/Shanghai --restart=always --name db-service -v /myProject/db-service:/usr/src/app -w /usr/src/app node:18-alpine sh -c "npm install && npm run prod"
```
