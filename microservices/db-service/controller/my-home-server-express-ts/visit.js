/**
 * 创建时间: 2025-06-01
 * 作者: jjq
 * 描述: visit 控制层
 */

const R = require('../../lib/index').R
const visitService = require('../../service/index').visitService

module.exports = class VisitController {
    static async getAllVisit(req) {
        try {
            return R.ok(200, '获取访问记录成功', await visitService.getAllVisit(req))
        } catch (error) {
            return R.err(500, '获取访问记录失败', String(error))
        } /* 注：Error 对象默认没有定义 toJSON() 方法，所以 JSON.stringify(error) 会返回 {}（空对象），导致无法看到错误信息。此处使用String(错误对象)来查看错误信息 */
    }
    static async getVisitByLimit(req) {
        try {
            return R.ok(200, '获取近期访问记录成功', await visitService.getVisitByLimit(req))
        } catch (error) {
            return R.err(500, '获取近期访问记录失败', String(error))
        } /* 注：Error 对象默认没有定义 toJSON() 方法，所以 JSON.stringify(error) 会返回 {}（空对象），导致无法看到错误信息。此处使用String(错误对象)来查看错误信息 */
    }

    static async keepLatestVisit() {
        try {
            return R.ok(200, '定期删除访问记录成功', await visitService.keepLatestVisit()) 
        } catch (error) {
            return R.err(500, '定期删除访问记录失败', String(error))
        }
    }
}
